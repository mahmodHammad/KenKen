...
export default function App() { 
    ...
    const algorithmOptions = [
        "Backtracking","Backtracking with forward checking","Backtracking with arc consistency"
    ]
...
useEffect(() => {
  ...
  const hasNeigbour = ([row,col],group)=>{ 
    ...
    let hasLeft= sameRow.length>1 ? (sameRow.findIndex(s=>col==s[1]+1) !==-1 ? 0 : 1):1
    let hasTop = sameCol.length>1?(sameCol.findIndex(s=>row==s[0]+1)!==-1 ? 0 : 1):1
...
<Box sx={{
    ...
    {renderBoard ?<div>
        <Box sx={{
            ...
            {randGenerateSize(size).map((row,rowIndex)=><Grid key={rowIndex} container >
        ...
        <Box sx={{marginTop:"60px", display:"flex",flexDirection:"row",
         justifyContent:"space-between"}}>
            <CustomSelect value={SelectedAlgo} handleSelect={handleSelectAlgo} options={algorithmOptions} title="Solving algorithm"/>
            {/* <CustomSelect options={mode} title="Mode"/> */}
           {SelectedAlgo!==''?<Button onClick={restart?handlerestart:handleSolveCLicked} variant="contained" sx={{padding:"14px 80px",fontSize:"20px"}} > {restart?"Restart": "Solve"} </Button> :null}
        </Box>
    </div>
    :<Box sx={{width:"600px"}}> 
            <CustomSelect value={size} handleSelect={handleSelectSize} options={gridSizes}  title="Grid size"/>
            <CustomSelect value={mode} handleSelect={handleSelectMode} options={modes} title="Modes"/>
            {size!=='' && mode!=='' ?<Button fullWidth onClick={StartGame} variant="contained" sx={{padding:"14px 80px",fontSize:"20px"}} > Solve </Button> :null}
